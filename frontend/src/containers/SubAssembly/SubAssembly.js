...
const SubAssembly = () => {
  ...
  const fetchSubAssemblyData = () => {
    axios
      ...
      .then((response) => {
        setData(response.data.panelPDC);
      })
      .catch((error) => {
        ...
        <div className="flex justify-center bg-background border-none">
          <div className="w-3/4 p-6 shadow-lg bg-white rounded-md my-8">
            <p className="text-4xl text-signature font-black mb-5 mt-3 flex items-center justify-center">
              {" "}
              Sub-Assembly{" "}
              <span className="p-2 ml-3 pl-4 pr-4 bg-red-600 text-xl text-white rounded-full">
                {section}
              </span>
            </p>
                    ...
                    {data &&
                      data.map((row) => (
                        <TableRow key={row.id} className="hover:bg-gray-100">
                          <TableCell align="center"> {row.panelId}</TableCell>
