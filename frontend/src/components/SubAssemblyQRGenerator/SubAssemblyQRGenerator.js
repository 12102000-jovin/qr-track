...
const SubAssemblyQRGenerator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const links = Array.from({ length: numQR }, (_, index) => ({
      link: `http://localhost:${applicationPortNumber}/${section}/${selectedWorkOrderId}/${selectedPDCId}/PANEL${formatPanelId(
        Number(startNum) + index
      )}`,
      generatedDate: moment()
        .tz("Australia/Sydney")
        .format("YYYY-MM-DD HH:mm:ss"),
      panelId: `PANEL${formatPanelId(Number(startNum) + index)}`,
    }));
  ...
  const handleDownload = async (panelId) => {
    const qrCodeData = qrCode.find((code) => code.panelId === panelId);
        ...
        {qrCode.map((code, index) => (
          ...
          >
            <div id={`qrcode-${code.panelId}`}>
              ...
              />
              <div className="mb-5">Sub-Assembly ID: {code.panelId}</div>
              <img
                src={imageData[code.panelId]}
                alt={`Convereted ${code.panelId}`}
